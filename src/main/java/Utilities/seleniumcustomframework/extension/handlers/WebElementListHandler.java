...
public class WebElementListHandler implements InvocationHandler, Refreshable {
    ...
    private By by;
    private final FrameWeapper frame;
    private WebDriverFrameSwitchingOrchestrator webDriverFrameSwitchingOrchestrator;
    private ArrayList<WebElement> webElements;

    public WebElementListHandler(DependencyInjector driver, SearchContext searchContext, By by, FrameWeapper frame, WebDriverFrameSwitchingOrchestrator webDriverFrameSwitchingOrchestrator) {
        this.driver=driver;
    ...
    private WebElement getWebElementProxy(WebElement webElement){
        WebElementHandler handler=new WebElementHandler(this.driver,this.searchContext, By.id("Dummy page locator - it is not being used a we're passing Webelement anyway"), this.frame, this.webDriverFrameSwitchingOrchestrator, webElement);
